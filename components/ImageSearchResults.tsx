...
const ImageSearchResults = (props: ImageSearchResultsProps) => {
  ...
  return (
    <div className="sm:pb-24 pb-40 mt-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 space-x-4">
        {searchData.items.map((result, index) => (
            ...
            <div className="group">
              <Link href={result.image.contextLink}>
                <img
                  src={result.link}
                  alt={result.title}
                  className="h-60 group-hover:shadow-xl w-full object-contain transition-shadow duration-200"
                />
              </Link>
              <Link href={result.image.contextLink}>
